import { waitForServices } from './utils/healthCheck.helper';
...
export default async function globalSetup() {
  ...
  try {
    ...
    const healthResponse = await testContext.languageDetectionClient.get(
      '/health'
    );

    if (healthResponse.status !== 200) {
      throw new Error(
        `Language Detection Service health check failed: ${healthResponse.status}`
      );
    }
